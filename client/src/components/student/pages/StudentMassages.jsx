...
const StudentMessages = () => {
  ...
  const filterItems = (items) => {
    ...
    return items.filter(item =>
      ...
      (item.createdBy?.fullName && item.createdBy.fullName.toLowerCase().includes(term)) ||
      (item.createdBy?.username && item.createdBy.username.toLowerCase().includes(term))
    );
  };
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'ongoing':
        return <span className="bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs">Ongoing</span>;
      case 'ended':
        return <span className="bg-gray-100 text-gray-800 px-2 py-1 rounded-full text-xs">Ended</span>;
      default:
        return <span className="bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full text-xs">Upcoming</span>;
    }
  };
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredAnnouncements.map(item => (
                ...
                >
                  ...
                  <p className="text-gray-600 line-clamp-2 mb-3">{item.message}</p>
                  <div className="text-sm text-gray-500 flex justify-between items-center">
                    <span><FiUser className="inline mr-1" /> {item.createdBy?.fullName || item.createdBy?.username}</span>
                    <span><FiCalendar className="inline mr-1" /> {formatDate(item.createdAt)}</span>
                  </div>
                </div>
              ))}
            </div>
              ...
              {filteredEvents.map(item => (
                ...
                >
                  ...
                  <div className="text-sm text-gray-500 grid grid-cols-2 gap-2">
                    <span><FiCalendar className="inline mr-1" /> {formatDate(item.date)}</span>
                    <span><FiClock className="inline mr-1" /> {item.startTime} - {item.endTime}</span>
                    <span><FiUser className="inline mr-1" /> {item.createdBy?.fullName}</span>
          ...
          {selectedItem && (
            <>
              ...
              <div className="mt-4 space-y-2 text-sm text-gray-600">
                {selectedItem.date && (
                  <p><FiCalendar className="inline mr-1" /> {formatDate(selectedItem.date)}</p>
                )}
                {selectedItem.startTime && (
                  <p><FiClock className="inline mr-1" /> {selectedItem.startTime} - {selectedItem.endTime}</p>
                )}
                {selectedItem.createdBy?.fullName && (
                  <p><FiUser className="inline mr-1" /> {selectedItem.createdBy.fullName}</p>
                )}
                {selectedItem.createdAt && (
                  <p>Posted on: {new Date(selectedItem.createdAt).toLocaleString()}</p>
                )}
                {selectedItem.status && (
